...
############################ HELPER FUNCTIONS ##################################
    ...
    def load(self):
        with open("walls.txt", 'r') as file:
            self.maze = file.readlines()
            self.maze = np.array([row.rstrip('\n') for row in self.maze])
    def transform_maze(self):
        np.random.shuffle(self.maze)
        for idx, x in enumerate(self.maze):
            if idx == 0 or idx == len(self.maze) - 1:
                self.maze[idx] = '1111111111111111111111111111'
            for jdx, y in enumerate(x):
                if jdx == 0 or jdx == len(self.maze[0]) - 1:
                    self.maze[idx] = self.maze[idx][:jdx] + '1' + self.maze[idx][jdx + 1:]
        if not any('P' in sublist for sublist in self.maze):
            self.maze[2] = self.maze[2][:2] + 'P' + self.maze[2][2:]
    def set_maze(self):
        for yidx, line in enumerate(self.maze):
            for xidx, char in enumerate(line):
                if char == "1":
                    self.walls.append(vec(xidx, yidx))
    ...
    def draw_maze(self):
        for yidx, line in enumerate(self.maze):
            for xidx, char in enumerate(line):
                if char == "1":
                    pygame.draw.rect(self.background, RED, (xidx * self.cell_width, yidx * self.cell_height,
                                                            self.cell_width, self.cell_height))
